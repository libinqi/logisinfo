...
module.exports = {
    index: function (req, res, next) {
        ...
        var model = {
            filter: {
                portlevel: Number(req.query.portlevel) || '',
                porttype: Number(req.query.porttype) || '',
                sberth: Number(req.query.sberth) || '',
                eberth: Number(req.query.eberth) || '',
                scity: '',
        ...
        if (model.filter.portlevel) {
            cl.SetFilter('portLevelCode', [model.filter.portlevel]);
        }
        if (model.filter.porttype) {
            cl.SetFilter('portTypeCode', [model.filter.porttype]);
        }
        if (model.filter.sberth && model.filter.eberth) {
            cl.SetFilterRange('berthNumber', model.filter.sberth, model.filter.eberth);
        }
        else if (model.filter.sberth) {
            cl.SetFilterRange('berthNumber', model.filter.sberth, 9999999999);
        } else if (model.filter.eberth) {
            cl.SetFilterRange('berthNumber', 0, model.filter.eberth);
        }

        var keyword = '';
        if (model.filter.scity && model.filter.scity != '全国') {
//            keyword = '@(portName,address) ' + model.filter.scity;
            cl.SetFilterString('city', model.filter.scity);
        }

        cl.Query(keyword, model.index, function (err, result) {
            if (err || !result || result.total == 0) {
                return res.render('port/index', {
                    title: '港口信息',
                    model: model
                });
            }
        ...
        cl.Query('', model.index, function (err, result) {
            if (err || result.matches.length == 0) {
                ...
                async.series([function (callback) {
                    ...
                    return res.render('port/detail', {
                        title: '港口详情',
                        port: result.matches[0]
                    });
                });
            }
        });
