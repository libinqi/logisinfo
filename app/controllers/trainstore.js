...
module.exports = {
    index: function (req, res, next) {
        ...
        var model = {
            filter: {
                trainstorelevel: Number(req.query.trainstorelevel) || '',
                trainstoretype: Number(req.query.trainstoretype) || '',
            ...
            index: 'logistics_trainstore_list',
        ...
        if (model.filter.trainstorelevel) {
            cl.SetFilter('trainStoreLevelCode', [model.filter.trainstorelevel]);
        }
        if (model.filter.trainstoretype) {
            cl.SetFilter('trainStoreTypeCode', [model.filter.trainstoretype]);
        }
        if (model.filter.sarea && model.filter.earea) {
            cl.SetFilterRange('trainStoreArea', model.filter.sarea, model.filter.earea);
        }
        else if (model.filter.sarea) {
            cl.SetFilterRange('trainStoreArea', model.filter.sarea, 9999999999);
        } else if (model.filter.earea) {
            cl.SetFilterRange('trainStoreArea', 0, model.filter.earea);
        }

        var keyword = '';
        if (model.filter.scity && model.filter.scity != '全国') {
//            keyword = '@(trainStoreName,address) ' + model.filter.scity;
            cl.SetFilterString('city', model.filter.scity);
        }

        cl.Query(keyword, model.index, function (err, result) {
            if (err || !result || result.total == 0) {
                return res.render('trainstore/index', {
                    title: '货场信息',
                    model: model
                });
            }
    ...
    detail: function (req, res, next) {
        ...
        cl.Query('', model.index, function (err, result) {
            if (err || result.matches.length == 0) {
                return res.redirect('/trainstore');
            }
            else {
                async.series([function (callback) {
                    ...
                    return res.render('trainstore/detail', {
                        title: '货场详情',
                        trainstore: result.matches[0]
                    });
                });
            }
        });
